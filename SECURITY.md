...
|---|---|---|
| Malicious settings file | Attacker writes crafted `settings.json` | Settings deserialized to a POCO (`BlinkSettings`) via `System.Text.Json` with no code execution; corrupt or unparseable JSON falls back to safe defaults. Out-of-range field values (e.g. interval < 1 or > 60) are not re-rejected at load time but have no dangerous effect — the timer behaviour is bounded by the OS and the app's own logic. `BlinkSettingsValidator` enforces the same rules in tests. |
